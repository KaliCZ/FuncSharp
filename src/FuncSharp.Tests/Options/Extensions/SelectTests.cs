using System;
using FsCheck;
using FsCheck.Xunit;
using FuncSharp.Tests.Generative;
using Xunit;

namespace FuncSharp.Tests.Options;

public class SelectTests
{
    public SelectTests()
    {
        Arb.Register<OptionGenerators>();
    }

    [Fact]
    internal void Select()
    {
        // Empty option mapped is always empty
        OptionAssert.IsEmpty(Option.Empty<int>().Select(v => v * 2));
        OptionAssert.IsEmpty(Option.Empty<string>().Select(v => (int?)null));

        // Valued option mapped will always have value
        OptionAssert.NonEmptyWithValue(84, 42.ToValuedOption().Select(v => v * 2));
        OptionAssert.NonEmptyWithValue(84, 42.ToValuedOption().Select(v => v * 2 as int?));
        OptionAssert.NonEmptyWithValue("xxxxx", 5.ToValuedOption().Select(v => new String('x', v)));

        // Even if you map to null, the option is still valued
        OptionAssert.NonEmpty(5.ToValuedOption().Select(v => null as int?));
        OptionAssert.NonEmpty(5.ToValuedOption().Select(v => (string?)null));
    }

    [Property]
    internal void Select_int(Option<int> option)
    {
        AssertSelectResult(option, i => i * 2);

        // Also mapping to a reference type.
        AssertSelectResult(option, i => new ReferenceType(i * 2));
    }

    [Property]
    internal void Select_decimal(Option<decimal> option)
    {
        AssertSelectResult(option, d => d * 2);
    }

    [Property]
    internal void Select_double(Option<double> option)
    {
        AssertSelectResult(option, d => d * 2);
    }

    [Property]
    internal void Select_bool(Option<bool> option)
    {
        AssertSelectResult(option, b => !b);
    }

    [Property]
    internal void Select_ReferenceType(Option<ReferenceType> option)
    {
        AssertSelectResult(option, d => new ReferenceType(d.Value * 2));

        // Also mapping to a struct
        AssertSelectResult(option, d => d.Value * 2);
    }

    private void AssertSelectResult<T, TResult>(Option<T> option, Func<T, TResult> map)
    {
        var result = option.Select(map);
        Assert.Equal(option.IsEmpty, result.IsEmpty);
        if (option.NonEmpty)
        {
            Assert.Equal(map(option.Get()), result.Get());
        }

        var linqResult =
            from x in option
            select map(x);
        Assert.Equal(result, linqResult);
    }
}