...
public class TryTests
{
    ...
    {
        var r1 = Try.Aggregate(Success, Success, (s1, s2) => (s1, s2));
        Assert.Equal(42, r1.Success.Get().s1);
        Assert.Equal(42, r1.Success.Get().s2);

        var r2 = Try.Aggregate(Success, Error, (s1, s2) => (s1, s2));
        Assert.Equal(Exception, r2.Error.Get().First());

        var r3 = Try.Aggregate(Error, Error, (s1, s2) => (s1, s2));
